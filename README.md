# JWilliker's [NixOS]  & [Home Manager] Configurations

[NixOS]: https://nixos.org/
[Home Manager]: https://github.com/nix-community/home-manager

This repository contains a [Nix Flake](https://nixos.wiki/wiki/Flakes) for configuring my computers and home lab.
This repository is derived from [wimpysworld/nix-config](https://github.com/wimpysworld/nix-config).
These are the computers this configuration currently manages:

|    Hostname    |       OEM      |        Model        |       OS      |     Role     |  Status  |
| :------------: | :------------: | :-----------------: | :-----------: | :----------: | :------- |
| `meerkat`      | System 76      | [NUC5i7RYH]         | NixOS         | Server       | WIP      |
| `cm4-io-01`    | Raspberry Pi   | Raspberry Pi 4      | NixOS         | Server       | tbn      |
| `cm4-io-02`    | Raspberry Pi   | Raspberry Pi 4      | NixOS         | Server       | tbn      |
| `cm4-io-03`    | Raspberry Pi   | Raspberry Pi 4      | NixOS         | Server       | tbn      |
| `pi400`        | Raspberry Pi   | Raspberry Pi 4      | NixOS         | Server       | tbn      |
| `pinebook-pro` | Pine64         | Pinebook Pro        | -             | Laptop       | tbn      |
| `star64`       | Pine64         | Star64              | -             | Laptop       | tbn      |
...
```bash
curl -sL https://raw.githubusercontent.com/jwillikers/nix-config/main/scripts/install.sh | bash -s <hostname> <username>
```
...
```bash
gh repo clone jwillikers/nix-config ~/Zero/nix-config
```
...
A live image will be left in `~/$HOME/Zero/nix-config/result/iso/`. These .iso images are also periodically built and published via [GitHub Actions](./.github/workflows) and available in [this project's Releases](https://github.com/jwillikers/nix-config/releases).
...
- [x] Create standard directories in `/home/jordan` using `systemd.user.tmpfiles.rules`.
- [x] Create `.envrc` files in `/home/jordan`.
